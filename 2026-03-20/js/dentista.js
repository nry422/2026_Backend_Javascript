...
frm.addEventListener("submit", (e) => {
    ...
    frm.inPaciente.value = ""; //limpa o conteudo do campo de formulario
    frm.inPaciente.focus(); //posiciona o cursos no campo
});

//adiciona um "ouvinte" para o evento click no btUrgencia que esta no from
frm.btUrgencia.addEventListener("click", () => {
    //verifica se as valicadoes do form estao ok (no caso, paciente is required)
    ...
    let lista = ""; // string para concatenar pacientes
    //forEach aplica sobre o array pacientes
    pacientes.forEach((paciente, i) => (lista += `${i+1}. ${paciente}\n`));
    respLista.innerText = lista; //exibe lista de pacientes na pagina
    frm.inPaciente.value = ""; // limpa o conteudo do campo de formularo
